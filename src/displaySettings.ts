...
export function getDisplaySettingsUpdateFromHomeSettings(
  value: unknown,
  current: QdnDisplaySettings,
): QdnDisplaySettings | null {
  if (!isObject(value)) {
  ...
  const language = normalizeLanguage(value.language ?? value.lang);
  const textSize = normalizeTextSize(value.textSize);
  const theme = normalizeTheme(value.theme);
  const uiStyle = normalizeUiStyle(value.ui ?? value.uiStyle);

  if (!accent && !language && !textSize && !theme && !uiStyle) {
    return null;
  }
  ...
  return {
    ...
    uiStyle: uiStyle ?? current.uiStyle,
  };
}
