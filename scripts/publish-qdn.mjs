...
const DEFAULT_NAME = 'Notify';
const DEFAULT_IDENTIFIER = 'Notify';
const DEFAULT_TITLE = 'Notify';
const DEFAULT_DESCRIPTION = 'Manage Qortium Home notification access and background rules.';
...
const nodeApiUrl = (process.env.QORTIUM_NOTIFY_NODE_API_URL ?? DEFAULT_NODE_API_URL).replace(/\/+$/, '');
const publishName = process.env.QORTIUM_NOTIFY_QDN_NAME ?? DEFAULT_NAME;
const identifier = process.env.QORTIUM_NOTIFY_QDN_IDENTIFIER ?? DEFAULT_IDENTIFIER;
const publishTitle = process.env.QORTIUM_NOTIFY_QDN_TITLE ?? DEFAULT_TITLE;
const service = process.env.QORTIUM_NOTIFY_QDN_SERVICE ?? 'APP';
const distPath = path.resolve(repoRoot, process.env.QORTIUM_NOTIFY_DIST_PATH ?? 'dist');
const apiKeyPath = expandHomePath(
  process.env.QORTIUM_NOTIFY_NODE_API_KEY_PATH ?? '~/.config/qortium-core/runtime/apikey.txt',
);
const previewAccountsPath = expandHomePath(
  process.env.QORTIUM_NOTIFY_PREVIEW_ACCOUNTS_PATH ??
...
function getApiKeySource() {
  const explicitApiKey = process.env.QORTIUM_NOTIFY_NODE_API_KEY?.trim();
  ...
  if (process.env.QORTIUM_NOTIFY_NODE_API_KEY_PATH?.trim()) {
    return {
...
// dist/ is gitignored, so refuse to publish a missing or stale bundle before
// loading any key material or contacting the node.
...
// The helper sends a preview account private key to the selected node for
// signing. Never do that over remote plaintext HTTP without an explicit opt-in.
if (
  !isLoopbackNodeApiUrl() &&
  !nodeApiUrl.startsWith('https://') &&
  process.env.QORTIUM_NOTIFY_ALLOW_REMOTE_SIGN !== '1'
) {
  throw new Error(
    `Refusing to send the account private key to non-loopback HTTP node ${nodeApiUrl}. ` +
      'Use a local node or an https:// URL, or set QORTIUM_NOTIFY_ALLOW_REMOTE_SIGN=1 to override.',
  );
}
